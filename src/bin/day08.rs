...
fn main() {
    const INPUT: &str = include_str!("../doc/day08.txt");
            ...
            for antenna_locations in lookup_cache.values() {
                for antenna1 in antenna_locations {
                    for antenna2 in antenna_locations {
                        ...
                        antinode_locations.insert(antenna1.clone()); // an antenna is also a antinode point now

                        let mut iteration = 1;
                        loop {
                            let x_diff: i32 = (antenna1.x as i32) - (antenna2.x as i32);
                            let new_x: i32 = (antenna1.x as i32) + x_diff * iteration;
                            if new_x < 0 || new_x >= map_size.width  as i32 { break; }

                            let y_diff: i32 = (antenna1.y as i32) - (antenna2.y as i32);
                            let new_y: i32  = (antenna1.y as i32) + y_diff * iteration;
                            if new_y < 0 || new_y >= map_size.height as i32 { break; }
