...
mod part2 {
    ...
    pub fn solve(input: &str) -> usize { 
        ...
        let mut round = 6493 - 25;

        for _ in 0..round  { for robot in robots.iter_mut() { robot.simulate_movement(); } }

        loop {
            for robot in robots.iter_mut() { robot.simulate_movement(); }
            println!();
            println!("Right now it's round {}:", round);
            debug_bathroom(&robots);
            println!();
            print!("Is this a easter-egg being displayed? (y)es / (N)o ");
            let mut io_in = String::new();
            io::stdin().read_line(&mut io_in).expect("Failed to read line");
            let io_in = io_in.to_lowercase();
            match io_in.as_str() {
                "y" | "yes" => { return round; },
                "n" | "no" => { continue; }
                _ => println!("That is not one of the options")
            }
            round += 1;
        }
    }

    /// Prints the bathrooms current state
    fn debug_bathroom(robots: &Vec<Robot>) {
        let mut bathroom = vec![vec![0; BATHROOM_DIMENSIONS.width as usize]; BATHROOM_DIMENSIONS.height as usize];
        println!("bathroom = ({}, {})", bathroom[0].len(), bathroom.len());
        for robot in robots {
            bathroom[robot.position.y as usize][robot.position.x as usize] += 1;
        }
